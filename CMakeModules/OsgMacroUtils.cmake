...
MACRO(ADD_OSG_APP  SUBDIR)
  SET(TARGET "${SUBDIR}_app")
	FILE(GLOB APP_SRC ${OPENSCENEGRAPH_DIR}/applications/${SUBDIR}/*.cpp)
	FILE(GLOB APP_H ${OPENSCENEGRAPH_DIR}/applications/${SUBDIR}/*.h)
	ADD_EXECUTABLE(${TARGET} ${APP_SRC} ${APP_H})
	SET_TARGET_PROPERTIES(${TARGET} PROPERTIES DEBUG_POSTFIX ${CMAKE_DEBUG_POSTFIX})
	SET_TARGET_PROPERTIES(${TARGET} PROPERTIES OUTPUT_NAME ${SUBDIR})
	MESSAGE(STATUS "adding osg app ${TARGET}")
	#orig#TARGET_LINK_LIBRARIES(${TARGET} osg osgDB osgUtil osgProducer ${ARGV1} ${ARGV2} ${ARGV3} ${ARGV4} ${ARGV5})
#	IF(UNIX)
#		MESSAGE("so no qui!!!!!!!!!!")
		FOREACH(LINKLIB osg osgDB osgGA osgUtil osgText OpenThreads ${ARGV1} ${ARGV2} ${ARGV3} ${ARGV4} ${ARGV5})
			IF(${LINKLIB} MATCHES "osg")	
				#MESSAGE("TARGET_LINK_LIBRARIES(${TARGET} ${LINKLIB}")
			...
			ELSE(${LINKLIB} MATCHES "osg")	
			 ...
			 ELSE(${LINKLIB} MATCHES "Producer")	
			  IF(${LINKLIB} MATCHES "OpenThreads")	
				#MESSAGE("TARGET_LINK_LIBRARIES(${MYTARGET} ${LINKLIB}")
				IF(MSVC)
					TARGET_LINK_LIBRARIES(${TARGET} optimized "${LINKLIB}Win32" debug "${LINKLIB}Win32d" )
				ELSE(MSVC)	
				  TARGET_LINK_LIBRARIES(${TARGET} optimized ${LINKLIB} debug "${LINKLIB}${CMAKE_DEBUG_POSTFIX}")
        ENDIF(MSVC)
		...
		ENDFOREACH(LINKLIB)
#	ELSE(UNIX)
#		TARGET_LINK_LIBRARIES(${TARGET} osg osgDB osgUtil osgProducer ${ARGV1} ${ARGV2} ${ARGV3} ${ARGV4} ${ARGV5})
#	ENDIF(UNIX)
	SET_TARGET_PROPERTIES(${TARGET} PROPERTIES PROJECT_LABEL "Application ${TARGET}")
	SET_TARGET_PROPERTIES(${MYTARGET} PROPERTIES DEBUG_POSTFIX ${CMAKE_DEBUG_POSTFIX})
	#speed#TARGET_LOCATIONS_ACCUM(${TARGET})	
	MESSAGE(STATUS "install_target->${TARGET}<--")
	#old form#INSTALL_TARGETS(/bin ${TARGET} )
	INSTALL(TARGETS ${TARGET} RUNTIME DESTINATION  bin  )
ENDMACRO(ADD_OSG_APP)

MACRO(ADD_OSG_EXAMPLE  SUBDIR)
	#PROJECT("Example_${SUBDIR}")
  SET(MYTARGET ${SUBDIR}_example)
...
MACRO(ADD_OSG_PLUGIN SUBDIR)
	...
	FILE(GLOB PLUGIN_SRC ${OPENSCENEGRAPH_DIR}/src/osgPlugins/${SUBDIR}/*.cpp)
	FILE(GLOB PLUGIN_H ${OPENSCENEGRAPH_DIR}/src/osgPlugins/${SUBDIR}/*.h)
