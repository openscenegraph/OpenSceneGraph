...
*  a specific geometry creation routine.
	*/
	...
	virtual osg::Geometry * makeWallGeometry(const osg::Vec3Array *points) const;
	...
	virtual osg::Vec3Array *getWall(const osg::Vec3Array *points,const float height) const;
	virtual osg::Vec2Array *getWallTexcoords(const osg::Vec3Array *points,const float height) const;
	virtual osg::Vec3Array *getWallNormals(const osg::Vec3Array *points) const {
...
public:
	...
	*/
	...
	virtual osg::Vec3Array *getRoadVertices(const osg::Vec3Array *points) const;
...
public:
	...
	virtual osg::Geometry * makeGeometry(const osg::Vec3Array *points) const
        {
...
{ // create a terrain tile. This is just an example!
	...
	if (ndcs>0) {
		for ( std::vector < pyramid* >::iterator itr=pyrlist.begin(); itr!=pyrlist.end(); itr++) {
			...
			geode->addDrawable((*itr)->makeGeometry(points)); // this fills the holes of each pyramid with geometry
		}
		...
		if (ndcs>2) {
			...
			geode->addDrawable(wc->makeWallGeometry(points)); // this creates wall at wc drawarrays
			if (ndcs>3) {
...
osg::Vec3Array * WallConstraint::getWall(const osg::Vec3Array *points,const float height) const
...
osg::Vec2Array * WallConstraint::getWallTexcoords(const osg::Vec3Array *points,const float height) const
{ // return array of points for a wall height high around the constraint
...
osg::Geometry *WallConstraint::makeWallGeometry(const osg::Vec3Array *points) const
{
	...
	gm->setVertexArray(getWall(points,height));
	gm->addPrimitiveSet(makeWall());
	gm->setTexCoordArray(0,getWallTexcoords(points,height));
	gm->setNormalBinding(osg::Geometry::BIND_PER_VERTEX);
	gm->setNormalArray(getWallNormals(points)); // this creates normals to walls
	
	return gm.release();
}

osg::Vec3Array *ArealConstraint::getWallNormals(const osg::Vec3Array *points) const
{
...
osg::Vec3Array * ArealConstraint::getWall(const osg::Vec3Array* points,const float height) const
{ // return array of points for a wall height high around the constraint
	osg::Vec3Array *wall=new osg::Vec3Array;
	if (height>0.0) {
		osg::Vec3 off(0,0,height);
...
osg::Vec2Array * ArealConstraint::getWallTexcoords(const osg::Vec3Array *points,const float height) const
{ // return array of points for a wall height high around the constraint
	osg::Vec2Array *tcoords= NULL;
	if (height>0.0) {
		float texrepRound=txxrepWall;
...
osg::Vec3Array *LinearConstraint::getRoadVertices(const osg::Vec3Array *points) const
{
