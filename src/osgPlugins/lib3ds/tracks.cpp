...
lib3ds_quat_track_read(Lib3dsQuatTrack *track, FILE *f)
{
    int keys;
    int i,j;
    // removed p definiation since MipsPro says that it ain't used..
    //Lib3dsQuatKey *p;
    Lib3dsQuatKey *k;
...
{
    // parameters lib3ds_morph_track_read(Lib3dsMorphTrack *track, FILE *f)
    /* FIXME: */
    return(LIB3DS_TRUE);
}
...
{
    // parameters : lib3ds_morph_track_write(Lib3dsMorphTrack *track, FILE *f)
    /* FIXME: */
    ASSERT(0);
    return(LIB3DS_FALSE);
}
