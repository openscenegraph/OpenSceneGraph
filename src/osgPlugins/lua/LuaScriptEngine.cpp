...
bool LuaScriptEngine::getelements(int pos, int numElements, int type) const
{
    ...
    for(int i=0; i<numElements; ++i)
    {
        ...
        //type control TOUNCOMMENT: can't remember if lua_pop should be i+2 or i+1 if (lua_type(_lua, -1)!=type) { lua_pop(_lua, i+1); return false; }
    }
    return true;
}
...
void LuaScriptEngine::pushValue(const osg::Matrixf& value) const
{
    ...
    for(unsigned int r=0; r<4; ++r)
    {
        ...
        {
            lua_pushnumber(_lua, 1+r*4+c); lua_pushinteger(_lua, (lua_Integer) value(r,c)); lua_settable(_lua, -3);
        }
    }
}
