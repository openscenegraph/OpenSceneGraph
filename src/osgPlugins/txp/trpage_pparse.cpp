    ...
    if (obj) {
        ...
        if (tok == TRPGTILEHEADER) {
            ...
            for (int i=0;i<numMat;i++) {
                ...
                int totSize;
                trpgrImageHelper *imageHelper = parse->GetImageHelp();
                int numImages=1;
                locMat.GetNumLocals(numImages);
                for (int imgN=0;imgN<numImages;imgN++) {
                    // read all the images for each local material
                    imageHelper->GetNthImageInfoForLocalMat(&locMat,imgN,&baseMat,&baseTex,totSize);
                    ...
                    // Fetch the whole image
                    {
                        ...
                        bool failed = false;
                        try {
                            failed = !imageHelper->GetNthImageForLocalMat(&locMat,imgN,pixels,totSize);
                        }
                        catch (...) {
                            failed = true;
                        }
                    ...
                    // Fetch the individual mipmap levels
                    {
                        ...
                        for (int j=1;j<numMipmap;j++) {
                            ...
                            if (mipSize) {
                                ...
                                bool failed = false;
                                try {
                                    failed = !imageHelper->GetNthImageMipLevelForLocalMat(j,&locMat,imgN,pixels,mipSize);
                                }
                                catch (...) {
                                    failed = true;
                                }
