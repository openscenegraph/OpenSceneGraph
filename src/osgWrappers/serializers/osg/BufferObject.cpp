...
namespace BufferDataWrapper
{
    ...
    static bool readBufferObject( osgDB::InputStream& is, osg::BufferData& node )
    {
        unsigned int size = 0;

        osg::ref_ptr<osg::Object> obj = is.readObject();
        osg::BufferObject* bo = dynamic_cast<osg::BufferObject*>( obj.get() );
        if ( bo ) node.setBufferObject(bo,false);///don't add BufferData to BufferObject (let Serializer do it)
        return true;
    }
